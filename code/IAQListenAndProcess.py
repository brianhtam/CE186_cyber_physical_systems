...
def storeresults(results, t, postStrings,keyStrings):

    for i=3      #i=len(results):     #i = 2 to 4, category of data
        for j=len(results[keyStrings[i]])
        result-obj = postStrings[1][1]
        result-stream = postStrings[1][1]
        target = 
        endpoint = '/networks/local/objects/' + result-obj + '/streams/' + result-stream + '/points'
        print results[target]
        query = {
            'points-value': results[target],
            'points-at': t
        }
...
def loop(timeL,tLE,tT,rN,resultsLast):
    ...
    ####### Aggregate Data
        ...
        if readingNo % 144 == 0:
            dayAvgStream = (PM/readingNo)+avgPM*(readingNo-1)/readingNo
        ...
        results = {"Average Levels": avg,
                   "Health Level": healthLevel,
                   "Percent Poor":percentPoorTime,
